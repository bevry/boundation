...
const { repoToOrganisation, repoToProject } = require('./string')

async function getGitOriginUrl (cwd = process.cwd()) {
	const detail = (details[cwd] = details[cwd] || {})
	if (detail.origin) return detail.origin
	try {
		const stdout = await exec('git remote get-url origin', { cwd, stdio: ['ignore', 'pipe', 'ignore'] })
...
async function getGitUsername (cwd = process.cwd()) {
	...
	try {
		const stdout = await exec('git config --global user.name', { cwd, stdio: ['ignore', 'pipe', 'ignore'] })
		const result = (stdout && stdout.toString().trim()) || null
...
async function getGitBranch (cwd = process.cwd()) {
	...
	try {
		const stdout = await exec('git rev-parse --abbrev-ref HEAD', { cwd, stdio: ['ignore', 'pipe', 'ignore'] })
		const result = (stdout && stdout.toString().trim()) || null
...
async function getGitEmail (cwd = process.cwd()) {
	...
	try {
		const stdout = await exec('git config --global user.email', { cwd, stdio: ['ignore', 'pipe', 'ignore'] })
		const result = (stdout && stdout.toString().trim()) || null
		detail.email = result
		return result
	}
...
module.exports = { getGitOriginUrl, getGitOrganisation, getGitProject, getGitUsername, getGitBranch, getGitEmail }
